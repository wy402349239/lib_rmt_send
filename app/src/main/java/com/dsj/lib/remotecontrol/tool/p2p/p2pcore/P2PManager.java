...
public class P2PManager {
    ...
    public void start(P2PNeighbor melon, Melon_Callback melon_callback) {
        ...
        if (p2PHandler == null){
            p2PHandler = new MelonHandler(p2pThread.mLooper == null ? Looper.myLooper() : p2pThread.mLooper);
        }
     ...
     * 字符串接收回执
     *
     * @param entity 字符串信息实体类
     */
    public void sendReceiptStr(ParamStrEntity entity) {
        if (entity == null) {
            return;
        }
        p2PHandler.initSend();
        p2PHandler.send2Handler(P2PConstant.CommandNum.Receipt_STR_REQ,
                P2PConstant.Src.MANAGER, P2PConstant.Recipient.FILE_SEND, entity);
    }
            ...
            switch (msg.what) {
                ...
                case P2PConstant.CommandNum.SEND_STR_REQ: //收到请求字符串
                    if (manager.receiveFile_callback != null) {
                        ...
                        manager.receiveFile_callback.ReceiverStr(params);
                    }
                    break;
                case P2PConstant.CommandNum.Receipt_STR_REQ: //收到回执
                    if (manager.receiveFile_callback != null) {
                        ParamStrEntity params = (ParamStrEntity) msg.obj;
                        manager.receiveFile_callback.ReceiptStr(params);
                    }
                    break;
