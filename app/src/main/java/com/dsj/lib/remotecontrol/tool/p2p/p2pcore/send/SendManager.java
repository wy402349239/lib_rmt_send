...
import com.dsj.lib.remotecontrol.tool.p2p.p2pentity.param.ParamSendFiles;
import com.dsj.lib.remotecontrol.tool.p2p.p2pentity.param.ParamStrEntity;
import com.dsj.lib.remotecontrol.tool.p2p.p2pentity.param.ParamTCPNotify;
...
public class SendManager {
        ...
        switch (src) {
            ...
            case P2PConstant.Src.MANAGER: {
                if (what == P2PConstant.CommandNum.SEND_FILE_REQ) {
                    ...
                    invokeFile(param.neighbors, param.files);
                } else if (what == P2PConstant.CommandNum.SEND_STR_REQ) {
                    if (!mSenders.isEmpty())
                        return;
                    ParamStrEntity entity = (ParamStrEntity) obj;
                    invoke(entity.getNeighbor(), entity.getContent(), what);
                } else if (what == P2PConstant.CommandNum.Receipt_STR_REQ) {
                    if (!mSenders.isEmpty())
                        return;
                    ParamStrEntity entity = (ParamStrEntity) obj;
                    invoke(entity.getNeighbor(), entity.getContent(), what);
    ...
    private void invoke(P2PNeighbor neighbors, String msg, int cmd) {
        if (neighbors == null){
            return;
        }
        ...
        //cmd = P2PConstant.CommandNum.SEND_STR_REQ || P2PConstant.CommandNum.Receipt_STR_REQ
        if (melon != null) {//通知对方，我要发送文件了
            if (p2PHandler != null)
                p2PHandler.send2Receiver(melon.inetAddress, cmd, msg);
        }
    }

    private void invokeFile(P2PNeighbor[] neighbors, P2PFileInfo[] files) {
